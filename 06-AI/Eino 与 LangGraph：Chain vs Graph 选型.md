...
## 7. 落地路线图：由点到线，由静态到动态（新手避坑）

新手搭建 LLM 服务最容易掉入的陷阱是一上来就搞 Graph + 复杂多轮问答，导致调试地狱（分不清是 LLM 没理解、Prompt 组装错了、还是状态流转断了）。推荐分 4 个阶段渐进式搭建：

1. **单轮 MVP 链**：不加历史记录、不加向量库，先用 Chain 把"接收请求 → 组装 Prompt → 调用 LLM → SSE 流式返回"跑通。用 curl/Postman 验证文字能逐字吐出。
2. **接入 1~2 个静态 Tool**：硬编码一个控制函数（如 `ControlAC(temp int)`），注册为 Tool 绑定到 LLM 节点，验证特定意图能正确触发该函数。
3. **叠加"记忆"实现多轮问答**：引入 Redis/MySQL，请求携带 `session_id`，读取最近 N 条历史消息和当前提问一起传入 Chain，回答后异步写回历史。验证"我叫张三"之后问"我刚才说我叫什么"能正确回答。
4. **配置化与动态加载**：把第二阶段硬编码的 Tool 抽离为数据库存 JSON Schema + 通用转发 Handler，服务启动时动态构建 Tool 数组（见 [[LLM Tool Calling 落地：CLI 与配置化 Agent]]）。

完成四阶段后已能满足 90% 的知识库问答/硬件控制/业务 Agent 需求。只有出现**内部自我修正**（如 CLI 执行失败需要带着报错信息跳回上一步重试）或**分支审批流**（高危命令需要人工在 Admin 点"同意"后才恢复执行）时，才需要把 Chain 重构成 Graph。

## 8. Chain 支持"中途改变主意"式的打断吗

不支持，普通 Chain 是无法天然打断并回退状态的单向同步流：从请求发起到执行 CLI/MQTT 控制命令通常几秒内就串行跑完，用户"转头又改主意"时其实已经是下一轮独立请求了。工程上两种解法：

- **方案 1：响应延迟/确认队列**（成本最低，多数场景够用）：Go 后端不立刻把指令发给硬件，先放入一个带 2~3 秒延迟的队列（Redis/Go Channel）。如果用户在窗口期内发了新指令（如"算了别开了"），大模型识别出撤销意图，后端直接从队列里撤销尚未真正发射的指令，或用新指令覆盖旧指令。
- **方案 2：Graph 的 Human-in-the-Loop 挂起/恢复**（仅高危操作需要）：Graph 运行到执行节点前自动挂起并保存 State，等待确认或打断信号，确认后才 Resume 真正下发。

打断还分两种物理层意图：**语义打断**（"算了别开了"，用方案 1 的覆盖/取消）vs **物理打断**（"停！"，需要本地检测到打断信号后向云端发 `kill_current_task` 强制终止当前 Goroutine）。常规口误场景不需要改动 Eino 架构，只有长时间运行的复杂 Agent 流程才需要 Graph 的中断能力。

## 9. 集成设备一多，是否必须上 Graph 才能处理"关机（不知道关哪个）"这种歧义指令

不需要。设备数量增多只是增加了 Prompt 里 Tool 定义的长度，数据流转依然是单向 Chain，只要每个 Tool 的 description 写清楚，大模型本身就是一个很强的路由选择器。真正需要解决的是**意图消歧义与多轮确认**，三层递进解法：

1. **System Prompt 约束**：指令模糊（只说"关机"未指明设备）时严禁调用任何 Tool，必须先自然语言反问，标准 Chain + Redis 历史存储就能跑通，零额外开发成本。
2. **带入当前设备状态**：请求时把 `currently_running_devices` / `off_devices` 作为上下文喂给大模型——只有一台设备开着时直接精准关闭；多台开着时精确追问"电脑和空调都在运行，关哪一个"，而不是把所有设备都报一遍。
3. **高危设备二次确认**：在 Tool 配置里加 `need_confirm: true` 标记，Go 服务拦截到高危 Tool Call 时先推送确认询问，收到用户确认后才真正下发。

只有当消歧义逻辑变成复杂的多阶段状态机（如"语义解析 → 检查高危 → 挂起等待人工审批 → 执行"，或需要重试/回滚机制）时，才需要 Graph。
